...
def main():
    ...
    for mn in ['p11', 'p01', 'p00']:
        print('modelnum = ',mn)
        filename = f'~/QuAN/Figure/Data_out/{args.saveprefix}/TC{args.dataformat}{nr*nc}_{mn}_0.000vs{p2train:.3f}_5_phasediagram.npz'
        if is_non_zero_file(filename):
            phase_diagram_out = np.zeros((9,howmanyruns,20,65))
            phase_diagram_pred = np.zeros((9,howmanyruns,20,65))
            data = np.load(filename)
            phase_diagram_out[:,:data['arr0'].shape[1]] = data['arr0']
            phase_diagram_pred[:,:data['arr0'].shape[1]] = data['arr1']
        else:
            phase_diagram_out = np.zeros((9,howmanyruns,20,65))
            phase_diagram_pred = np.zeros((9,howmanyruns,20,65))
        args.modelnum = mn
        for k, setsize in enumerate(2**np.arange(7)):
            for run in range(howmanyruns):
                ...
                args.ch = 1
                args.ker = 1
                args.batchsize = int(4096//setsize)
                ...
                for i, bx in enumerate(np.arange(20)*0.02):
                    bz = 0.14
                    for j, pflip in enumerate(np.arange(65)*0.005):
                        ...
                        if x1_bxbz.shape[0]>2:
                            ...
                            phase_diagram_out[k, run, i,j] = mat_output.mean(axis=0)
                            phase_diagram_pred[k, run, i,j] = mat_pred.mean(axis=0)
                            del test_inputs, test_outputs
                        else: print('No data', (bx,bz), pflip)
                print(setsize, run, modelloadname)
        np.savez(filename, arr0=phase_diagram_out, arr1=phase_diagram_pred)
