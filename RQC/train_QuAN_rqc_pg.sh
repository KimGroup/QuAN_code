#############################################################################
# training with experimental data for SM section D4
#############################################################################
model_name=$1
nr=$2
nc=$3
...
do
    ...
    -set $setsize -n_mini $n_mini -d1 $depth -d2 20 -nr $nr -nc $nc -nsy 'google' \
    ...
    -wandb_name '${model_name}_A2_pg_${nr}x${nc}_${depth}_vs20_run_${run}' \
    $additional "
    echo $cmd
    eval $cmd
done
