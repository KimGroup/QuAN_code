...
class Classifier(nn.Module):
    """
    Encoder from Vaswani et al (2017) adapted for quantum trajectory classification task
    """
...
class Generator(nn.Module):
    '''
    Map Classifier output to probabilities. Linear + softmax.
    '''
...
class PositionalEncoding1D(nn.Module):
    '''
    Trig-based positional encodingn for 1D input.
    '''
...
class PositionalEncoding2D(nn.Module):
    '''
    Trig-based positional encoding for 2D input.
    '''
...
class EncoderLayer(nn.Module):
    '''
    Encoder is made up of 1 self-attention (SA) layer and 1 feed-forward (FF) layer.
    '''
    ...
    def forward(self, x, mask=None):
        '''
        Pass the input through each module in the sublayer (SA and FF)
        '''
...
# Label smoothing: The model learns to be more unsure.
