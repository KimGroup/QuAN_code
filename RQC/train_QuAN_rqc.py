...
def get_arguments():
    ...
    parser.add_argument("-saveprefix", help = "save folder prefix", type = str, default = '~/QuAN/Data_out/g2_saved_models_rqc')
...
def main():
    ...
    model = torch.nn.DataParallel(model); print('model loaded.')
    ...
    if args.nr==6: stepsize = args.lrstepsize//2
    else: stepsize = args.lrstepsize
    ...
    for epoch in range (total_epoch):
        ...
        for j,valdata in enumerate(val_loader):
            ...
            val_outputs = model.module(val_inputs.float())
            del val_inputs
            ...
            m0 = val_labels.squeeze() < 0.5 # shallow depth recall
            m1 = val_labels.squeeze() > 0.5 # deep depth recall
        ...
        if best_acc_v < val_acc:
            torch.save(model.module.state_dict(), f'{args.saveprefix}/{args.wandb_name}/model_va_{model_name}.pth')
            best_acc = val_acc
        ...
        if best_acc_t < test_acc:
            torch.save(model.module.state_dict(), f'{args.saveprefix}/{args.wandb_name}/model_ta_{model_name}.pth')
            best_acc_t = test_acc
        if best_loss_t > test_loss:
            torch.save(model.module.state_dict(), f'{args.saveprefix}/{args.wandb_name}/model_tl_{model_name}.pth')
