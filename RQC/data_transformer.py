...
def gen_data_simulated(args, Nt, c_p):
    ...
    num_circ = 50 # Nc = 50
    num_train_circ = 35 # 35 training circuit instance
    circnum = np.arange(num_circ) 
    circnum = circnum[c_p]
    testcirc = circnum[num_train_circ:] # 15 testing circuit instance
    ...
    if Nt>30: n_bitstring = 500000*4 # Ms = 2,000,000 for N_q = 36
    else: n_bitstring = 500000 # Ms = 500,000 for N_q = 20, 25, 30
...
def gen_data_simulated_20vs20(args, Nt, c_p):
    ...
    x3v, x3t = x3[:,:x3.shape[1]//2], x3[:,x3.shape[1]//2:]
    x4v, x4t = x4[:,:x4.shape[1]//2], x4[:,x4.shape[1]//2:]
    print(f'gen_data_20vs20: test on {testcirc[:]}')
    print(f'shapes: {x1.shape}, {x3v.shape}, {x3t.shape}')
    return x1, x2, x3v, x4v, x3t, x4t
...
def gen_data_google_20vs20(args, Nt, c_p):
    ...
    testcirc = circnum[35:]
    traincirc = np.setdiff1d(circnum, testcirc)
    if Nt>30: n_bitstring = 500000
    else: n_bitstring = 250000
    ...
    x3v, x3t = x3[:,:x3.shape[1]//2], x3[:,x3.shape[1]//2:]
    x4v, x4t = x4[:,:x4.shape[1]//2], x4[:,x4.shape[1]//2:]
    print(f'gen_data_google_20 vs 20: test on {testcirc[:]}')
    print(f'shapes: {x1.shape}, {x3v.shape}, {x3t.shape}')
    return x1, x2, x3v, x4v, x3t, x4t
