...
def gen_data_n8_entangle(args, c_p):
    ...
    '''
    num_circ = 17 # Nc = 17
    num_train_circ = 12 # 12 training circuit instance
    circnum = np.arange(num_circ)+14
    circnum = circnum[c_p]
    testcirc = circnum[num_train_circ:] # 5 testing circuit instance
    traincirc = np.setdiff1d(circnum, testcirc) 
    
    path = '../Data_src/HCBH_dataset_n8'
    add_path = f'state_{args.basis}' # z-basis measurement
    n_bitstring = 4096//args.set * args.set # Ms = 4096
    nt = args.nr*args.nc # Nq = 16

    # volume-law (\delta=0) training data 
    ...
    # area-law (\delta=2) training data 
    ...
    # volume-law (\delta=0) testing data 
...
def gen_testdata_sample(set_size, x1, x2, Nt):
    ...
    '''
    n = 1
    xx1 = np.zeros((x1.shape[0]*int(n*x1.shape[1]/set_size), set_size, Nt), dtype=bool) #shallow depth training data
    xx2 = np.zeros((x2.shape[0]*int(n*x2.shape[1]/set_size), set_size, Nt), dtype=bool) #deep depth training data
    for i in range (n):
        p = np.random.permutation(x1.shape[1])
